...
MeshDataHandle<ColoredVertex> getCubeMesh() {
    ...
    if (cubeHandle.isInvalid()) {
        ...
        std::vector<uint32_t> indices{// Front face (z = +0.5)
                                    ...
                                    // Top face (y = +0.5)
                                    2, 6, 3, 3, 6, 7};
        cubeHandle = MeshData<ColoredVertex>::registerMeshData(MeshData(std::move(vertices), std::move(indices)));
    }
    return cubeHandle;
}
...
MeshDataHandle<UnlitVertex> getCubeMesh() {
    ...
    if (cubeHandle.isInvalid()) {
        ...
        cubeHandle = MeshData<UnlitVertex>::registerMeshData(MeshData(std::move(vertices), std::move(indices)));
    }
    return cubeHandle;
}
